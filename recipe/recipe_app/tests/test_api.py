...
def test_create_ingredient(auth_headers):
    url = f"{BASE_URL}/create_ingredient/"
    ...
    get_url = f"{BASE_URL}/get_ingredients/"
...
def test_create_recipe(auth_headers):
    ...
    ingredient_url = f"{BASE_URL}/create_ingredient/"
    ...
    get_ing_url = f"{BASE_URL}/get_ingredients/"
    ...
    recipe_payload = {
        ...
        "ingredients": [{"ingredient": flour["id"], "quantity": 500}],
    }
...
def test_upload_recipe_image(auth_headers):
    ...
    upload_url = f"{BASE_URL}/recipes/{recipe_id}/upload-image/"
...
def test_create_recipe_with_image(auth_headers):
    # Create an in-memory image (PNG)
    img_bytes = io.BytesIO()
    image = Image.new('RGB', (100, 100), color='blue')  # Blue image 100x100
    image.save(img_bytes, format='PNG')
    img_bytes.seek(0)

    # Form URL for recipe creation
    recipe_url = f"{BASE_URL}/recipes/"

    # Recipe data
    recipe_payload = {
        "name": "Blueberry Pancakes",
        "description": "Delicious pancakes with blueberries",
        "ingredients": [],  # No ingredients for now
    }

    # Form multipart request data
    files = {"image": ("pancakes.png", img_bytes, "image/png")}

    # Send POST request to create recipe with image
    recipe_response = requests.post(recipe_url, data=recipe_payload, files=files, headers=auth_headers)
    assert recipe_response.status_code == 201, f"Recipe creation error: {recipe_response.text}"
    recipe_data = recipe_response.json()
    recipe_id = recipe_data.get("id")
    assert recipe_id, "Recipe ID not returned"
    assert recipe_data.get("name") == "Blueberry Pancakes", "Incorrect recipe name"
    assert recipe_data.get("description") == "Delicious pancakes with blueberries", "Incorrect recipe description"
    assert recipe_data.get("image"), "Recipe image not returned"
...
def test_edit_recipe(auth_headers):
    ...
    edit_url = f"{BASE_URL}/recipes/{recipe_id}/"
    updated_payload = {
        "name": "Updated Pancakes",
        "description": "Fluffy and tasty pancakes with milk",
        "ingredients": [{"ingredient": milk["id"], "quantity": 200}],
    }
    ...
    assert updated_data["name"] == "Updated Pancakes", "Recipe name was not updated"
    assert updated_data["description"] == "Fluffy and tasty pancakes with milk", "Recipe description was not updated"
    assert len(updated_data["ingredients"]) == 1, "Ingredients were not added"
    assert updated_data["ingredients"][0]["ingredient"]["name"] == "Milk", "Incorrect ingredient added"
    assert updated_data["ingredients"][0]["ingredient_amount"] == 200, "Incorrect ingredient quantity"
    print(f"✅ Recipe {recipe_id} successfully updated!")
